...
are auto-generated by running `tree-sitter generate --abi=14 --no-bindings`. (You need to
...
This grammar targets **Tree-sitter ABI version 14** for compatibility with the Zed editor
and other downstream tools. Zed currently supports ABI versions 13-14 (see [Zed issue #24632](https://github.com/zed-industries/zed/issues/24632)).

When generating the parser, always use the `--abi=14` flag:
```sh
tree-sitter generate --abi=14 --no-bindings
```

The build script (`scripts/build.js`) enforces this automatically.
...
* You ran `tree-sitter generate --abi=14 --no-bindings` in the `tree-sitter-pandoc-markdown` or
  `tree-sitter-pandoc-markdown-inline` directories respectively after modifying any
  `grammar.js` file (or use `npm run build` which handles this automatically).
